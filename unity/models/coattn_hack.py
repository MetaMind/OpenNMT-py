...
from modules import BiEncoder, BiCoattention, FancySelfAttention
import torch
...
class Model(BaseModel):
    ...
    def __init__(self, args, vocab, label_vocab):
        ...
        self.bienc_premise = BiEncoder(args.d_hid*2+args.d_hid*2, args.d_hid*2)
        self.bienc_hypothesis = BiEncoder(args.d_hid*2+args.d_hid*2, args.d_hid*2)
        self.self_attn_premise = FancySelfAttention(args.d_hid*4)
        self.self_attn_hypothesis = FancySelfAttention(args.d_hid*4)

        d_feat = args.d_hid * 4

        self.scorer = nn.Sequential(
            tnn.Linear(d_feat * 2, args.d_hid*2*2),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.BatchNorm1d(args.d_hid*2*2),
            tnn.Linear(args.d_hid*2*2, args.d_hid*2),
            nn.ReLU(),
            nn.Dropout(0.25),
            nn.BatchNorm1d(args.d_hid*2),
            tnn.Linear(args.d_hid*2, len(label_vocab))
        )
    ...
    def forward(self, batch):
        ...
        bienc_premise, _ = self.bienc_premise(torch.cat([coattn_premise, enc_premise], 2), batch.premise_lens)
        bienc_hypothesis, _ = self.bienc_hypothesis(torch.cat([coattn_hypothesis, enc_hypothesis], 2), batch.hypothesis_lens)
